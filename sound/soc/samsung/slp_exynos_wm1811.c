...
#define CODEC_DEFAULT_MCLK1     24000000
#define CODEC_DEFAULT_MCLK2     32768
...
struct slp_snd_board_info {
	...
	bool enforce_micbias2; /* only for wm1811 rev.B */
...
static struct slp_snd_board_info board_info[] = {
	...
	{ /* SND_BOARD_PQ_V00 */
		...
		{ true, true },
		true,
		{ GPIO_MIC_BIAS_EN_00, GPIO_SUB_MIC_BIAS_EN_00 },
		false,
		false,
	},
	{ /* SND_BOARD_PQ_V01 */
...
static const struct wm8958_micd_rate machine_det_rates[] = {
	{ CODEC_DEFAULT_MCLK2,		true,  0, 0 },
	{ CODEC_DEFAULT_MCLK2,		false, 0, 0 },
...
static int check_board(void)
{
	...
	if (machine_is_redwood()) {
		if (system_rev == 0x06) /* real rev 0.0 = 0x06 */
			return SND_BOARD_REDWOOD_R00;
		else if (system_rev == 0x07) /* real rev 0.1 = 0x07 */
			return SND_BOARD_REDWOOD_R01;
		else  /* universal board */
...
static void exynos_snd_set_mclk_forced(bool on)
{
	if (on) {
		/* enable mclk */
		dev_warn(&slp_snd_device->dev, "forced enable mclk");
...
static void exynos_snd_set_mclk(bool on)
{
	...
	if (exynos_snd_mclk_use_cnt) {
		if (!exynos_snd_mclk_enabled) {
			/* enable mclk */
			dev_warn(&slp_snd_device->dev, "enable mclk");
			exynos4_pmu_xclkout_set(exynos_snd_mclk_enabled,
						XCLKOUT_XUSBXTI);
			exynos_snd_mclk_enabled = true;
			mdelay(10);
		} else /* already enabled */
			dev_dbg(&slp_snd_device->dev, "mclk already enabled");
	} else {
		if (exynos_snd_mclk_enabled) {
			/* disable mclk */
			dev_warn(&slp_snd_device->dev, "disable mclk");
			exynos4_pmu_xclkout_set(exynos_snd_mclk_enabled,
						XCLKOUT_XUSBXTI);
			exynos_snd_mclk_enabled = false;
		} else
			dev_dbg(&slp_snd_device->dev, "mclk already disabled");
...
static void codec_start_fll1(struct snd_soc_dai *aif1_dai)
{
	int ret;

	if (codec_fll1_active)
		return;
	...
	ret = snd_soc_dai_set_pll(aif1_dai, WM8994_FLL1,
				  WM8994_FLL_SRC_MCLK1, CODEC_DEFAULT_MCLK1,
					...
					struct snd_pcm_hw_params *params)
{
	...
	if (aif2_mode == MODE_CODEC_SLAVE) {
		...
		pll_freq_in = CODEC_DEFAULT_MCLK1;
	}
...
const struct snd_soc_dapm_widget machine_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SPK("SPK", exynos_snd_spkmode),
...
static int machine_init_paiftx(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = snd_soc_dai_set_sysclk(aif1_dai, WM8994_SYSCLK_MCLK2,
				     CODEC_DEFAULT_MCLK2, SND_SOC_CLOCK_IN);
	if (ret < 0)
...
static int machine_card_suspend(struct snd_soc_card *card)
{
...
static int machine_card_resume(struct snd_soc_card *card)
{
	exynos_snd_set_mclk(true);
...
static int machine_set_bias_level(struct snd_soc_card *card,
				...
				enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		codec_start_fll1(card->rtd[0].codec_dai);
...
static int machine_set_bias_level_post(struct snd_soc_card *card,
		struct snd_soc_dapm_context *dapm,
		enum snd_soc_bias_level level)
{
	struct snd_soc_codec *codec = card->rtd->codec;
	...
	case SND_SOC_BIAS_STANDBY:
		...
		if (card->dapm.bias_level == SND_SOC_BIAS_PREPARE) {
			...
			codec_fll1_active = false;

			codec_micd_set_rate(codec);
			exynos_snd_set_mclk(false);
		}
		break;
	default:
		break;
...
static struct snd_soc_card exynos_wm1811_machine = {
	...
	.set_bias_level = machine_set_bias_level,
	.set_bias_level_post = machine_set_bias_level_post,
	.suspend_post = machine_card_suspend,
	.resume_pre = machine_card_resume
};
