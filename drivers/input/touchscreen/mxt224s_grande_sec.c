...
static void mxt_treat_dbg_data(struct mxt_data *data,
	struct object_t *dbg_object, u8 dbg_mode, u8 read_point, u16 num)
{
	struct i2c_client *client = data->client;
	struct mxt_data_sysfs *sysfs_data = data->sysfs_data;
	...
	if (dbg_mode == MXT_DIAG_DELTA_MODE) {
		/* read delta data */
		read_mem(data, dbg_object->i2c_address + read_point,
		...
		if (sysfs_data->reference[num] < MIN_VALUE
			|| sysfs_data->reference[num] > MAX_VALUE) {
			dev_err(&client->dev,
