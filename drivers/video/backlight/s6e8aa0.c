...
static int s6e8aa0_brightness_ctrl(struct s6e8aa0 *lcd, int brightness)
{
	int ret = 0;
...
static int s6e8aa0_panel_init(struct s6e8aa0 *lcd)
{
	...
	s6e8aa0_brightness_ctrl(lcd, brightness);
...
static int s6e8aa0_set_brightness(struct backlight_device *bd)
{
	...
	ret = s6e8aa0_brightness_ctrl(lcd, brightness);
	if (ret) {
...
static int s6e8aa0_set_dimming(struct backlight_device *bd)
{
	...
	dev_info(lcd->dev, "%s: brightness %ddimming %d.\n",
...
static int s6e8aa0_probe(struct mipi_dsim_lcd_device *dsim_dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(bd_device_attrs); i++) {
		ret = device_create_file(&lcd->bd->dev,
					&bd_device_attrs[i]);
		if (ret < 0) {
			dev_err(&lcd->bd->dev, "failed to add sysfs entries\n");
			break;
		}
	}
