...
#ifdef USE_INITIAL_2G_SCAN_ORG
		if ((request->n_channels > 10) && ndev == wl_to_prmry_ndev(wl) &&
...
#if defined(CUSTOMER_HW_SAMSUNG) && defined(WES_SUPPORT)
		else if (wl_cfg80211_is_wes(&mgmt_frame[DOT11_MGMT_HDR_LEN],
			mgmt_frame_len - DOT11_MGMT_HDR_LEN)) {
						...
						if (connected_cnt  > 1) {
							...
							if (wl->roamoff_on_concurrent) {
								err = wldev_iovar_getint(iter->ndev, "roam_off",
									(s32 *)&iter->roam_off);
								if (err == BCME_OK) {
									err = wldev_iovar_setint(iter->ndev, "roam_off", 1);
									if (err != BCME_OK) {
										WL_ERR((" failed to set roam_off : %d\n", err));
									}
								} else
									WL_ERR(("failed to get roam_off : %d\n", err));
							}
						}
					}
				}
