...
#endif /*defined(CONFIG_HAS_EARLYSUSPEND)||defined(CONFIG_PARTIALSUSPEND_SLP)*/
...
#elif defined(CONFIG_PARTIALSUSPEND_SLP)
	struct pre_suspend pre_suspend;
...
#elif defined(CONFIG_PARTIALSUSPEND_SLP)
static void dhd_pre_suspend(struct pre_suspend *h)
{
	struct dhd_info *dhd = container_of(h, struct dhd_info, pre_suspend);
...
static void dhd_post_resume(struct pre_suspend *h)
{
	struct dhd_info *dhd = container_of(h, struct dhd_info, pre_suspend);
...
#elif defined(CONFIG_PARTIALSUSPEND_SLP)
	dhd->pre_suspend.suspend = dhd_pre_suspend;
	dhd->pre_suspend.resume = dhd_post_resume;
	register_pre_suspend(&dhd->pre_suspend);
	dhd_state |= DHD_ATTACH_STATE_EARLYSUSPEND_DONE;
#endif
...
#elif defined(CONFIG_PARTIALSUSPEND_SLP)
	if (dhd->dhd_state & DHD_ATTACH_STATE_EARLYSUSPEND_DONE) {
		if (dhd->pre_suspend.suspend)
			unregister_pre_suspend(&dhd->pre_suspend);
	}
#endif /* defined(CONFIG_HAS_EARLYSUSPEND) */
