...
#ifdef CONFIG_VIDEO_SLP_S5C73M3
#define S5C73M3_FW_PATH_OPT_MEDIA		"/opt/usr/media/SlimISP.bin"
...
#define S5C73M3_CORE_VDD	"/opt/usr/media/ISP_CV"
...
static const struct s5c73m3_frmsizeenum preview_frmsizes[] = {
	...
	{ S5C73M3_PREVIEW_3264X1836,	3264,	1836,	0x0E },
	{ S5C73M3_PREVIEW_HDR,	3264,	2448,	0x0D },
};
...
static int s5c73m3_i2c_check_status(struct v4l2_subdev *sd)
{
	...
	if (index >= 2000) {
		...
		cam_err("TimeOut!! index:%d,status:%#x,i2c_stauts:%#x,i2c_seq_status:%#x\n",
			index,
			status,
			i2c_status,
			i2c_seq_status);
...
static int s5c73m3_dump_fw(struct v4l2_subdev *sd)
{
	...
	if (state->sensor_fw[0] == 'O') {
		sprintf(fw_path, "/opt/usr/media/SlimISP_G%c.bin",
			state->sensor_fw[1]);
	} else if (state->sensor_fw[0] == 'S') {
		sprintf(fw_path, "/opt/usr/media/SlimISP_Z%c.bin",
			state->sensor_fw[1]);
	} else {
		sprintf(fw_path, "/opt/usr/media/SlimISP_%c%c.bin",
		state->sensor_fw[0],
		state->sensor_fw[1]);
	}
	...
	err = s5c73m3_spi_read((char *)buf,	state->sensor_size, rxSize);
	CHECK_ERR(err);
	...
	vfs_write(fp, (char __user *)buf, state->sensor_size, &fp->f_pos);
...
static int s5c73m3_get_phone_fw_version(struct v4l2_subdev *sd)
{
	...
	retVal = s5c73m3_open_firmware_file(sd,
		S5C73M3_FW_PATH_OPT_MEDIA,
		buf,
		S5C73M3_FW_VER_FILE_CUR,
		S5C73M3_FW_VER_LEN);
	if (retVal >= 0) {
		cam_dbg("firmware in opt/media\n");
		camfw_info[S5C73M3_OPT_MEDIA].opened = 1;
		memcpy(camfw_info[S5C73M3_OPT_MEDIA].ver,
			buf,
			S5C73M3_FW_VER_LEN);
		camfw_info[S5C73M3_OPT_MEDIA]
			.ver[S5C73M3_FW_VER_LEN+1] = ' ';
		state->fw_index = S5C73M3_OPT_MEDIA;
		fw_requested = 0;
	} else {
		cam_dbg("No firmware in opt/media\n");
	}
	...
	if (fw_requested) {
		...
		sprintf(fw_path_in_phone, "/lib/firmware/%s", fw_path);
		retVal = s5c73m3_open_firmware_file(sd,
			fw_path_in_phone,
			buf,
			S5C73M3_FW_VER_FILE_CUR,
...
static int s5c73m3_check_fw(struct v4l2_subdev *sd, int download)
{
	...
	if (!download) {
		...
		if (err < 0)
			cam_err("get_phone_fw_version failed\n");
	}

	retVal = s5c73m3_check_fw_date(sd);

	/* Support for PQ_PROXIMA old target*/
	if ((state->sensor_fw[0] == 'Z') && (state->sensor_fw[1] == 'E'))
		retVal = -1;
...
static int s5c73m3_set_scene_mode(struct v4l2_subdev *sd, int val)
{
	...
	if (state->exif.effect != IMAGE_EFFECT_NONE) {
		err = s5c73m3_writeb(sd, S5C73M3_IMAGE_EFFECT,
				s5c73m3_effects[0].reg_val);
		CHECK_ERR(err);
	}
...
static int s5c73m3_load_fw(struct v4l2_subdev *sd)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	if (state->fw_index == S5C73M3_OPT_MEDIA) {

		fp = filp_open(S5C73M3_FW_PATH_OPT_MEDIA, O_RDONLY, 0);
		if (IS_ERR(fp))
			goto out;
		else
			cam_dbg("%s is opened\n", S5C73M3_FW_PATH_OPT_MEDIA);
		...
		fp = filp_open(fw_path_in_phone, O_RDONLY, 0);
		if (IS_ERR(fp))
			goto out;
		else
			cam_dbg("%s is opened\n", fw_path_in_phone);
...
static int s5c73m3_set_frmsize(struct v4l2_subdev *sd)
{
	...
	if (state->format_mode != V4L2_PIX_FMT_MODE_CAPTURE) {
		err = s5c73m3_writeb(sd, S5C73M3_CHG_MODE,
			S5C73M3_DEFAULT_MODE | state->preview->reg_val |
			(state->sensor_mode<<8));
		CHECK_ERR(err);
...
static int s5c73m3_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable == STREAM_MODE_CAM_ON) {
		if(state->preview->width >= 1280 && state->preview->height >= 720)
				s5c73m3_set_frame_rate(sd, S5C73M3_FIXED_15FPS);
	} else {
		s5c73m3_set_frame_rate(sd, S5C73M3_FIXED_30FPS);
	}
