...
#ifdef CONFIG_VIDEO_SLP_S5C73M3
#define S5C73M3_FW_PATH_OPT_MEDIA		"/opt/media/SlimISP.bin"
...
static int s5c73m3_dump_fw(struct v4l2_subdev *sd)
{
	...
	if (state->sensor_fw[0] == 'O') {
		sprintf(fw_path, "/opt/media/SlimISP_G%c.bin",
			state->sensor_fw[1]);
	} else if (state->sensor_fw[0] == 'S') {
		sprintf(fw_path, "/opt/media/SlimISP_Z%c.bin",
			state->sensor_fw[1]);
	} else {
		sprintf(fw_path, "/opt/media/SlimISP_%c%c.bin",
		state->sensor_fw[0],
		state->sensor_fw[1]);
	}
