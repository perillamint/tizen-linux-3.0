...
static const struct m5mo_frmsizeenum capture_frmsizes[] = {
	...
	{ M5MO_CAPTURE_HD,		1280,	720,		0x10},
	{ M5MO_CAPTURE_2MP,	1600,	1200,	0x17},
	{ M5MO_CAPTURE_FULLHD,	1920,	1080,	0x19},
	{ M5MO_CAPTURE_W2MP,	2048,	1232,	0x2C},
	{ M5MO_CAPTURE_3MP,	2048,	1536,	0x1B},
	{ M5MO_CAPTURE_W4MP,	2560,	1536,	0x1D},
	{ M5MO_CAPTURE_5MP,	2560,	1920,	0x1F},
	{ M5MO_CAPTURE_6MP,	3264,	1960,	0x22},
...
static int m5mo_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
...
#ifdef CAM_CTRL_Debug
	cam_err("[m5mo_g_ctrl] id %d cmd, orig id %x\n",
		ctrl->id - V4L2_CID_PRIVATE_BASE, ctrl->id);
#endif

	switch (ctrl->id) {
	case V4L2_CID_CAMERA_SENSOR_MODE:
		ctrl->value = state->sensor_mode;
		break;

	case V4L2_CID_CAM_DR:		/* Fix me, it will remove */
	...
	case V4L2_CID_PHOTOMETRY:	/* Fix me, it will remove */
	case V4L2_CID_CAMERA_METERING:
		ctrl->value = state->metering;
		break;

	case V4L2_CID_EXPOSURE:		/* Fix me, it will remove */
	case V4L2_CID_CAMERA_BRIGHTNESS:
		ctrl->value = state->exposure;
		break;

	case V4L2_CID_SATURATION:	/* Fix me, it will remove */
	case V4L2_CID_CAMERA_SATURATION:
		ctrl->value = state->saturation;
		break;

	case V4L2_CID_SHARPNESS:	/* Fix me, it will remove */
	case V4L2_CID_CAMERA_SHARPNESS:
		ctrl->value = state->sharpness;
		break;

	case V4L2_CID_WHITE_BALANCE_PRESET:	/* Fix me, it will remove */
	...
	default:
		...
		err = 0;
#ifdef CAM_CTRL_Debug
		cam_err("[m5mo_g_ctrl]no such control id %d cmd, orig id %x\n",
				ctrl->id - V4L2_CID_PRIVATE_BASE, ctrl->id);
#endif
		break;
	}
...
#ifdef CAM_CTRL_Debug
	cam_err("[m5mo_g_ctrl] id %d cmd, orig id %x read value %d\n",
...
static int m5mo_check_fw(struct v4l2_subdev *sd)
{
	...
	err = m5mo_readb(sd, M5MO_CATEGORY_ADJST,
		M5MO_ADJST_AWB_RG_H, &rg_cal_h);
	CHECK_ERR(err);
	err = m5mo_readb(sd, M5MO_CATEGORY_ADJST,
		M5MO_ADJST_AWB_RG_L, &rg_cal_l);
	CHECK_ERR(err);

	err = m5mo_readb(sd, M5MO_CATEGORY_ADJST,
		M5MO_ADJST_AWB_BG_H, &bg_cal_h);
	CHECK_ERR(err);
	err = m5mo_readb(sd, M5MO_CATEGORY_ADJST,
		M5MO_ADJST_AWB_BG_L, &bg_cal_l);
	CHECK_ERR(err);
...
static int m5mo_set_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	struct v4l2_queryctrl qc = {0,};
	int val = ctrl->value, err;
	u32 iso[] = {0x01, 0x01, 0x02, 0x03, 0x04, 0x05};
...
static int m5mo_set_lock(struct v4l2_subdev *sd, int val)
{
	...
	val = 0; /* Fix me Lock Scenario is not full matched with U1HD */
			/* Tempoary set unlock only */
...
retry:
	...
	default:
		...
		cam_warn("**********************");
		cam_warn("WARN: CHECK FOCUS MODE");
		cam_warn("**********************");
		/* FIXME */
		val = FOCUS_MODE_AUTO;
		goto retry;
	...
	if ((status & 0x01) != 0x00) {
		cam_err("failed\n");
		/*return -ETIMEDOUT;*/
		/*This return value cause camera lock-up.*/
	}
...
static int m5mo_start_capture(struct v4l2_subdev *sd, int val)
{
	...
	if (!(state->isp.int_factor & M5MO_INT_CAPTURE)) {
		int_factor = m5mo_wait_interrupt(sd,
			state->face_beauty ?
			M5MO_ISP_AFB_TIMEOUT : M5MO_ISP_TIMEOUT);
...
static int m5mo_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
...
#ifdef CAM_CTRL_Debug
	cam_err("[m5mo_s_ctrl] id %d cmd, orig id %x, Set Value\n",
		ctrl->id - V4L2_CID_PRIVATE_BASE, ctrl->id, ctrl->value);
#endif
	...
	case V4L2_CID_FOCUS_AUTO_MODE:	/* Fix me, it will remove */
	case V4L2_CID_CAMERA_FOCUS_MODE:
	...
	default:
		...
		err = 0;
#ifdef CAM_CTRL_Debug
		cam_err("[m5mo_s_ctrl]no such control id %d cmd, orig id %x\n",
				ctrl->id - V4L2_CID_PRIVATE_BASE, ctrl->id);
#endif
		break;
	}
...
static int m5mo_g_ext_ctrl(struct v4l2_subdev *sd,
...
static int m5mo_g_ext_ctrls(struct v4l2_subdev *sd,
...
request_fw:
	...
	offset = SZ_64K * 31;
	if (id == 0x01) {
		err = m5mo_program_fw(sd, buf + offset,
			M5MO_FLASH_BASE_ADDR + offset, SZ_8K, 4, id);
	} else {
		err = m5mo_program_fw(sd, buf + offset,
			M5MO_FLASH_BASE_ADDR + offset, SZ_4K, 8, id);
	}
...
static int m5mo_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *a)
{
	...
	/* FIXME */
	if (a->parm.capture.capturemode)
		err = m5mo_writeb(sd, M5MO_CATEGORY_PARM,
			M5MO_PARM_FLEX_FPS, 0);
	else
		err = m5mo_writeb(sd, M5MO_CATEGORY_PARM,
...
static int m5mo_s_stream_hdr(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		/* convert raw to jpeg by the image data processing and store
		 * memory on ISP and receive preview jpeg image from ISP */
		for (i = 0; i < 3; i++) {
			int_factor = m5mo_wait_interrupt(sd, M5MO_ISP_TIMEOUT);
		...
		/* stop ring-buffer */
		if (!(state->isp.int_factor & M5MO_INT_CAPTURE)) {
			/* FIXME - M5MO_INT_FRAME_SYNC interrupt
...
static int m5mo_init_param(struct v4l2_subdev *sd)
{
	...
	/* HDR */
	err = m5mo_writeb(sd, M5MO_CATEGORY_CAPCTRL,
		M5MO_CAPCTRL_CAP_FRM_COUNT, 0x03);
	CHECK_ERR(err);
...
static int m5mo_init(struct v4l2_subdev *sd, u32 val)
{
	...
	state->focus.mode = FOCUS_MODE_DEFAULT;
