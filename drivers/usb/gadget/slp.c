...
struct android_usb_function {
	...
	/* for android_dev.functions, Deprecated */
	struct list_head enabled_list;

	/* for android_dev.funcs_fconf */
	struct list_head fconf_list;

	/* for android_dev.funcs_sconf */
...
struct android_dev {
	...
	/* for simple configuration control, Deprecated */
			       ...
			       struct usb_configuration *c)
{
	...
	if (!list_empty(&adev->functions)) {
		list_for_each_entry(f, &adev->functions, enabled_list) {
			if ((f->usb_config_id == c->bConfigurationValue) ||
			    (f->usb_config_id == USB_CONFIGURATION_DUAL)) {
				printk(KERN_DEBUG "usb_bind_conf(com): %s f:%s\n",
					__func__, f->name);
				ret = f->bind_config(f, c);
				if (ret) {
					pr_err("%s: %s bind_conf(com) failed",
						__func__, f->name);
					return ret;
				}
			}
		...
		if (c->bConfigurationValue == USB_CONFIGURATION_1) {
			list_for_each_entry(f, &adev->funcs_fconf, fconf_list) {
				printk(KERN_DEBUG "usb_bind_conf(1st): %s f:%s\n",
					__func__, f->name);
				ret = f->bind_config(f, c);
				if (ret) {
					pr_err("%s: %s bind_conf(1st) failed",
						__func__, f->name);
					return ret;
				}
			}
		} else if (c->bConfigurationValue == USB_CONFIGURATION_2) {
			list_for_each_entry(f, &adev->funcs_sconf, sconf_list) {
				printk(KERN_DEBUG "usb_bind_conf(2nd): %s f:%s\n",
					__func__, f->name);
				ret = f->bind_config(f, c);
				if (ret) {
					pr_err("%s: %s bind_conf(2nd) failed",
						__func__, f->name);
					return ret;
				}
			}
...
#define ADD_FUNCS_LIST(head, member, alive)	\
static inline int add_##member(struct android_dev *adev, char *name)	\
{	\
	struct android_usb_function *av_f, *en_f;	\
	\
	pr_debug("usb: %s name=%s\n", __func__, name);	\
	list_for_each_entry(av_f, &adev->available_functions,	\
			available_list) {	\
		if (!strcmp(name, av_f->name)) {	\
			list_for_each_entry(en_f, &adev->head,	\
					member) {	\
				if (av_f == en_f) {	\
					pr_info("usb:%s already enabled!\n",\
						 name);	\
					return 0;	\
				}	\
			}	\
			list_add_tail(&av_f->member, &adev->head);	\
			return 0;	\
		}	\
	}	\
	return -EINVAL;	\
...
{	\
	...
	\
	if (!alive)	\
		pr_warn("[Deprecated:%s(USB)] Don't use this\n",	\
			__func__);	\
	\
	list_for_each_entry(f, &adev->head, member) {	\
		pr_debug("usb: %s enabled_func=%s\n",	\
		       __func__, f->name);	\
...
{	\
	...
	\
	if (adev->enabled) {	\
		pr_info("can't change usb functions"	\
			"(already enabled)!!\n");	\
		return -EBUSY;	\
	}	\
	\
	INIT_LIST_HEAD(&adev->head);	\
	\
	pr_debug("usb: %s buff=%s\n", __func__, buff);	\
...
ADD_FUNCS_LIST(funcs_fconf, fconf_list, true)
ADD_FUNCS_LIST(funcs_sconf, sconf_list, true)

/* Deprecated : functions was replaced with func_fconf & func_sconf */
ADD_FUNCS_LIST(functions, enabled_list, false)
			    ...
			    const char *buff, size_t size)
{
	...
	if (sysfs_streq(buff, "1"))
		...
		pr_err("%s: Invalid cmd %c%c..", __func__, *buff, *(buff+1));
		return -EINVAL;
	}
	...
	if (enabled && !adev->enabled) {
		...
		if (!list_empty(&adev->functions)) {
			pr_warn("[Deprecated control(USB)] Don't use 'functions'\n");
			list_for_each_entry(f, &adev->functions,
					enabled_list) {
...
static int __devinit slp_multi_probe(struct platform_device *pdev)
{
	...
	INIT_LIST_HEAD(&adev->functions);

	INIT_LIST_HEAD(&adev->funcs_fconf);
	INIT_LIST_HEAD(&adev->funcs_sconf);
