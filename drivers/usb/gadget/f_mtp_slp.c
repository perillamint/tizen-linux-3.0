...
/* This is exynos-udc dependent value */
#define MTP_BULK_MAX_SIZE		round_down((0x7FFFF), 512)
			  ...
			  size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		if (req) {
			(count > MTP_BULK_MAX_SIZE) ?
				(xfer = MTP_BULK_MAX_SIZE) : (xfer = count);
			tx_buf = PAGE_ALIGN(xfer);

			BUG_ON(req->buf);
			for (;;) {
				req->buf = kmalloc(tx_buf,
							(GFP_ATOMIC | GFP_DMA));
				...
				if (tx_buf > PAGE_SIZE) {
					pr_info("[%s] (%d) malloc fail, retry it!\n",
						__func__, xfer);
					tx_buf = PAGE_ALIGN((tx_buf >> 1));
					xfer = tx_buf;
				} else {
					pr_err("[%s] PAGE_SIZE malloc fail!!\n",
							__func__);
					(r > count) ?
						(r -= count) : (r = -ENOMEM);
					goto mtpg_write_terminate;
				}
			}
...
static long mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case SET_ZLP_DATA:
		...
		req->buf = kmalloc(bulk_in->maxpacket, GFP_KERNEL);
		if (!req->buf) {
			mtp_req_put(dev, dev->tx_idle, req);
			status = -ENOMEM;
			break;
...
static void mtpg_complete_in_free(struct usb_ep *ep, struct usb_request *req)
{
	struct mtpg_dev *dev = the_mtpg;
	struct f_mtp *mtp_func = ep->driver_data;
				 ...
				 struct usb_endpoint_descriptor *intr_desc)
{
	...
	for (i = 0; i < MTP_TX_REQ_MAX; i++) {
		...
		req->complete = mtpg_complete_in_free;
		mtp_req_put(dev, &mtp_func->bulk_in_q, req);
	}
