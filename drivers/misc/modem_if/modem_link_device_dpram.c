...
static inline void log_dpram_status(struct dpram_link_device *dpld, char *str)
{
	...
	pr_info("%s: %s: %s: [%02d:%02d:%02d.%03d] "
		...
		get_txq_head(dpld, IPC_FMT), get_txq_tail(dpld, IPC_FMT),
		get_rxq_head(dpld, IPC_FMT), get_rxq_tail(dpld, IPC_FMT),
		get_txq_head(dpld, IPC_RAW), get_txq_tail(dpld, IPC_RAW),
		get_rxq_head(dpld, IPC_RAW), get_rxq_tail(dpld, IPC_RAW),
		recv_intr(dpld));
}
...
static void save_dpram_dump_work(struct work_struct *work)
{
	...
	while (1) {
		...
		ts2utc(&mst->ts, &utc);
		snprintf(dump, PAGE_SIZE, "[%02d:%02d:%02d.%03d] "
			"ACC{%X %d} FMT{TI:%u TO:%u RI:%u RO:%u} "
			"RAW{TI:%u TO:%u RI:%u RO:%u} INTR{0x%X}\n",
			utc.hour, utc.min, utc.sec, utc.msec,
...
static int process_res_ack(struct dpram_link_device *dpld, int dev)
{
	...
	if (ret == -ENOSPC) {
		...
		*/
		...
		queue_delayed_work(ld->tx_wq, ld->tx_dwork[dev], delay);
	} else {
