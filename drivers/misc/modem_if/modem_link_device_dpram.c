...
/**
 * get_circ_pointer
 * @dpld: pointer to the instance of a dpram_link_device structure
 ...
 *
 * Returns the value of a pointer in a circular queue after verification
 */
static inline u32 get_circ_pointer(struct dpram_link_device *dpld, int id,
				int dir, int ptr, void __iomem *addr)
{
	...
	while (1) {
		/* Read values from the address and compare them */
		val = ioread16(addr);
		saved = ioread16(addr);
		if (likely(val == saved))
			break;
		...
		cnt++;
		mif_err("%s: ERR! %s_%s.%s val(%d) != saved(%d), count %d\n",
			ld->name, get_dev_name(id), circ_dir(dir),
			circ_ptr(ptr), val, saved, cnt);
...
{
	return get_circ_pointer(dpld, id, TX, HEAD, dpld->dev[id]->txq.head);
}
...
{
	return get_circ_pointer(dpld, id, RX, TAIL, dpld->dev[id]->rxq.tail);
}
 ...
 * set_dpram_map
 * @dpld: pointer to the instance of a dpram_link_device structure
 * @map: pointer to the instance of a mif_irq_map structure
 *
 ...
 * @work: pointer to the instance of a work_struct structure
 *
 ...
 * @ts: pointer to the instance of a timespec structure
 ...
 * @dpld: pointer to the instance of a dpram_link_device structure
 * @dev: IPC device (IPC_FMT, IPC_RAW, etc.)
 * @src: start address of the buffer in an RX circular queue
 * @qsize: size of the buffer in the queue
 * @in: value of the "in" (head) pointer in the queue
 * @out: value of the "out" (tail) pointer in the queue
 * @rcvd: size of data in the queue
 *
 ...
 * @arg: pointer to the instance of a dpram_link_device structure
 *
...
/**
 * ipc_rx_task
 * @data: pointer to the instance of a dpram_link_device structure
 *
 ...
 * @dpld: pointer to the instance of a dpram_link_device structure
 * @dev: IPC device (IPC_FMT, IPC_RAW, etc.)
 * @qsize: size of the buffer in a RXQ
 * OUT @in: pointer into which the value of head (in) pointer is stored
 * OUT @out: pointer into which the value of tail (out) pointer is stored
 *
 * Stores head & tail pointer values in the TXQ to the corresponsing variables
 * via in & out pointers in the list of parameters.
 *
...
#ifndef CONFIG_SAMSUNG_PRODUCT_SHIP
			log_dpram_status(dpld, "CP2MIF");
			save_ipc_trace(dpld, dev, src, qsize, in, out, rcvd);
			trigger_force_cp_crash(dpld);
#endif
			rcvd = -EBADMSG;
			goto exit;
 ...
 * @dpld: pointer to the instance of a dpram_link_device structure
 * @dev: IPC device (IPC_FMT, IPC_RAW, etc.)
 * @qsize: size of the buffer in a TXQ
 * OUT @in: pointer into which the value of head (in) pointer is stored
 * OUT @out: pointer into which the value of tail (out) pointer is stored
 *
 * Stores head & tail pointer values in the TXQ to the corresponsing variables
 ...
 * @dpld: pointer to the instance of a dpram_link_device structure
 * @dev: IPC device (IPC_FMT, IPC_RAW, etc.)
 * @qsize: size of the TXQ
 * @in: head (in) pointer == offset to the start of free space
 * @out: tail (out) pointer == offset to the end of free space
 * @skb: pointer to an instance of sk_buff structure
 *
 * Must be invoked only when there is enough space in the TXQ.
 */
static void write_ipc_to_txq(struct dpram_link_device *dpld, int dev, u32 qsize,
			u32 in, struct sk_buff *skb)
{
	struct link_device *ld = &dpld->ld;
...
static int xmit_ipc_msg(struct dpram_link_device *dpld, int dev)
{
	...
	while (1) {
		...
		write_ipc_to_txq(dpld, dev, qsize, in, skb);
		copied += skb->len;
 ...
 * @work: pointer to the instance of the work_struct structure
 *
 ...
 * @dpld: pointer to the instance of a dpram_link_device structure
 * @dev: IPC device (IPC_FMT, IPC_RAW, etc.)
 * @iod: pointer to the instance of the io_device structure
...
/**
 * dpram_send
 * @ld: pointer to the instance of the link_device structure
 * @iod: pointer to the instance of the io_device structure
 * @skb: pointer to an skb that will be transmitted
 *
