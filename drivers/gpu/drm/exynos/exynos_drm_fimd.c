...
static void fimd_iommu_init(struct fimd_context *ctx)
{
	...
	while (1) {
		...
		if (value == VIDCON1_VSTATUS_FRONTPORCH) {
			int ret;
...
static void fimd_apply(struct device *subdrv_dev)
{
	...
	for (i = 0; i < WINDOWS_NR; i++) {
		...
		if (win_data->enabled && ovl_ops) {
			if (ovl_ops->commit)
				ovl_ops->commit(subdrv_dev, i);

			if (ovl_ops->enable)
				ovl_ops->enable(subdrv_dev, i);
		}
	}
			      ...
			      struct exynos_drm_overlay *overlay)
{
	...
	DRM_DEBUG_KMS("%s:zpos[%d]\n", __func__, overlay->zpos);
...
static void fimd_win_enable(struct device *dev, int zpos)
{
	...
	DRM_DEBUG_KMS("%s:win[%d],suspended[%d]\n", __func__,
		win, ctx->suspended);
...
static void fimd_wait_for_vblank(struct device *dev)
{
	...
	DRM_DEBUG_KMS("suspended[%d]\n", ctx->suspended);

	if (ctx->suspended)
		return;

	/*
	 * shadow registers will be updated between vblank front portch
	 * and vsync so wait for vsync.
	 */
	ret = wait_for((__raw_readl(ctx->regs + VIDCON1) &
		VIDCON1_VSTATUS_VSYNC), FIMD_VSYNC_TIMEOUT);
	if (ret < 0)
