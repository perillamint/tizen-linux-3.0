	...
	enum drm_exynos_ipp_buf buf)
{
	...
	DRM_DEBUG_KMS("%s:buf_id[%d]buf[%d]\n", __func__,
		buf_id, buf);
			...
			enum drm_exynos_ipp_buf buf)
{
	...
	DRM_DEBUG_KMS("%s:prop_id[%d]buf_id[%d]buf[%d]\n", __func__,
		property->prop_id, buf_id, buf);
	...
	return gsc_src_set_buf_seq(ctx, buf_id, buf);
}
	...
	enum drm_exynos_ipp_buf buf)
{
	...
	/* interrupt enable */
	if (buf == IPP_BUF_ENQUEUE &&
	    gsc_dst_get_buf_seq(ctx) >= GSC_BUF_START)
		gsc_handle_irq(ctx, true, false, true);

	/* interrupt disable */
	if (buf == IPP_BUF_DEQUEUE &&
			...
			enum drm_exynos_ipp_buf buf)
{
	...
	return gsc_dst_set_buf_seq(ctx, buf_id, buf);
}
