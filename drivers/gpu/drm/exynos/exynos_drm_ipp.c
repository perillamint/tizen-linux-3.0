 ...
 * @complete: completion of end of command.
					...
					struct drm_file *file)
{
	...
	DRM_DEBUG_KMS("%s prop_list->ipp_id : %d\n",
					...
					struct drm_file *file)
{
	...
	DRM_INFO("%s:priv[0x%x]ippdrv[0x%x]c_node[0x%x]\n", __func__,
		(int)priv, (int)ippdrv, (int)c_node);
	...
	DRM_INFO("%s:prop_id[%d]cmd[%d] created.\n", __func__,
		property->prop_id, property->cmd);
	...
	init_completion(&c_node->complete);
	...
	*ipp_find_drv_node(u32 prop_id)
{
	...
	if (list_empty(&exynos_drm_ippdrv_list)) {
		DRM_INFO("%s:ippdrv_list is empty.\n",
			__func__);
		return NULL;
	}
...
static int ipp_check_mem_list(struct exynos_drm_ippdrv *ippdrv,
	struct drm_exynos_ipp_cmd_node *c_node)
{
...
static struct drm_exynos_ipp_mem_node
	*ipp_get_mem_node(struct drm_device *drm_dev, struct drm_file *file,
	struct drm_exynos_ipp_cmd_node *c_node,
	struct drm_exynos_ipp_queue_buf *qbuf)
{
	struct drm_exynos_ipp_mem_node *m_node;
	...
	struct drm_exynos_ipp_cmd_node *c_node)
{
	...
	DRM_DEBUG_KMS("%s:prop_id[%d]s\n", __func__, property->prop_id);

	/* store property in ippdrv */
	ippdrv->property = property;

	if (!ipp_check_mem_list(ippdrv, c_node)) {
...
static void ipp_sched_cmd(struct work_struct *work)
{
	struct drm_exynos_ipp_cmd_work *cmd_work =
		(struct drm_exynos_ipp_cmd_work *)work;
	struct exynos_drm_ippdrv *ippdrv = cmd_work->ippdrv;
	struct ipp_context *ctx = get_ipp_context(ippdrv->parent_dev);
	struct drm_exynos_ipp_cmd_node *c_node;
	struct drm_exynos_ipp_property *property;
	int ret;
	...
	/* ToDo: need ippdrv, ctx null check ? */

	/* find command node */
	c_node = ipp_find_id(&ctx->prop_idr, &ctx->prop_lock,
		cmd_work->prop_id);
	if (!c_node) {
		DRM_ERROR("invalid command node list:prop_id[%d]\n",
			cmd_work->prop_id);
		goto err_clear;
	}
	...
	if (c_node->state != IPP_STATE_START) {
		DRM_INFO("%s:bypass state[%d]prop_id[%d].\n",
			__func__, c_node->state, cmd_work->prop_id);
		goto err_clear;
	}
	...
	/* start property */
	ret = ipp_start_property(cmd_work->drm_dev, ippdrv, c_node);
	if (ret) {
	...
	if (property->cmd == IPP_CMD_M2M) {
		if (!wait_for_completion_timeout(&c_node->complete,
			msecs_to_jiffies(200))) {
			DRM_ERROR("timeout wait for completion:prop_id[%d]\n",
				cmd_work->prop_id);
			goto err_unlock;
		}
	}
	...
	int *buf_id)
{
	...
	DRM_DEBUG_KMS("%s:done cmd[%d]prop_id[%d]\n", __func__,
		property->cmd, property->prop_id);

	return 0;
}

static void ipp_sched_event(struct work_struct *work)
{
	...
	ctx = get_ipp_context(ippdrv->parent_dev);
	if (!ctx) {
		DRM_ERROR("failed to get context.\n");
		goto err_clear;
	}

	c_node = ipp_find_id(&ctx->prop_idr, &ctx->prop_lock,
		ippdrv->property->prop_id);
					...
					struct drm_file *file)
{
	...
	DRM_DEBUG_KMS("%s:prop_id[%d]ops_id[%s]buf_id[%d]buf[%d]\n",
		__func__, qbuf->prop_id, qbuf->ops_id ? "dst" : "src",
		qbuf->buf_id, qbuf->buf);
	...
	case IPP_BUF_ENQUEUE:
		...
		if (qbuf->ops_id == EXYNOS_DRM_OPS_DST) {
			...
			if (property->cmd == IPP_CMD_M2M) {
				...
				if (pm_runtime_suspended(ippdrv->dev)) {
					DRM_INFO("%s:suspended state.\n",
						__func__);
					break;
				}

				cmd_work = kzalloc(sizeof(*cmd_work),
					GFP_KERNEL);
				if (!cmd_work) {
					DRM_ERROR(
						"failed to alloc cmd work.\n");
					ret = -ENOMEM;
					goto err_clean_event;
				}

				INIT_WORK((struct work_struct *)cmd_work,
					ipp_sched_cmd);
				cmd_work->drm_dev = drm_dev;
				cmd_work->ippdrv = ippdrv;
				cmd_work->prop_id =  qbuf->prop_id;
				queue_work(ctx->cmd_workq,
					...
					struct drm_file *file)
{
	...
	case IPP_CTRL_PLAY:
		...
		if (!cmd_work) {
			DRM_ERROR("failed to alloc cmd work.\n");
			return -ENOMEM;
		}
		...
		cmd_work->drm_dev = drm_dev;
		cmd_work->ippdrv = ippdrv;
		cmd_work->prop_id =  cmd_ctrl->prop_id;
		queue_work(ctx->cmd_workq,
			(struct work_struct *)cmd_work);
		break;
	case IPP_CTRL_STOP:
		mutex_lock(&c_node->cmd_lock);
		ret = ipp_stop_property(drm_dev, ippdrv,
			c_node);
		if (ret) {
