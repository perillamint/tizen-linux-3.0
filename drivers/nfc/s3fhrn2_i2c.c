/*
 * SAMSUNG NFC S3FHRN2 Controller for I2C I/F
 *
...
struct s3fhrn2_info {
	...
	struct mutex mutex;
	enum s3fhrn2_state state;	/* off, normal, firm download mode */
...
static bool enabled;
int NFC_VDD_18V(int on , struct i2c_client *client)
{
	...
	regulator = regulator_get(NULL, "NFC_AVDD_1.8V");

	pr_info("NFC_VDD_18V\n");
	dev_dbg(&client->dev, "NFC_VDD_18V\n");

	if (IS_ERR(regulator)) {
		dev_dbg(&client->dev, "NFC_VDD_18V Error\n");
		return PTR_ERR(regulator);
	}

	if (on) {
		regulator_enable(regulator);
		dev_dbg(&client->dev, "NFC_VDD_18V On\n");
	} else {
		/* TODO: If there is a case the regulator must be disabled*/
		if (regulator_is_enabled(regulator)) {
			dev_dbg(&client->dev, "NFC_VDD_18V OFF\n");
...
static int s3fhrn2_set_state(struct s3fhrn2_info *info,
					enum s3fhrn2_state state)
{
	...
	msleep(S3FHRN2_VEN_WAIT_TIME);

	dev_dbg(&client->dev, "%s: state: 0x%x\n", __func__, state);
	...
	msleep(S3FHRN2_VEN_WAIT_TIME*2);
...
static int s3fhrn2_reset(struct s3fhrn2_info *info)
{
	...
	dev_err(&client->dev, "i2c failed. return restart to M/W\n");
			  ...
			  size_t count, loff_t *offset)
{
	...
	int min = S3FHRN2_MSG_MIN_SIZE;
	int max = S3FHRN2_MSG_MAX_SIZE;
	int ret = 0;
	...
	/* check the length value,
		...
		dev_err(&client->dev, "receive invalid length : %x\n",
			info->buf[0]);
...
static long s3fhrn2_ioctl(struct file *file, unsigned int cmd,
							unsigned long arg)
{
	struct s3fhrn2_info *info = container_of(file->private_data,
						struct s3fhrn2_info, miscdev);
...
static int s3fhrn2_resume(struct device *dev)
{
	...
	mutex_lock(&info->mutex);
		...
		resume sequence here

	mutex_unlock(&info->mutex);
	*/
	NFC_VDD_18V(1 , client);
		...
		const struct i2c_device_id *id)
{
	...
	NFC_VDD_18V(1 , client);
	dev_dbg(&client->dev, "%s: NFC_VDD_18V set 1\n" , __func__);

	gpio_set_value(pdata->ven, 1);
	msleep(50);

	ret = i2c_master_recv(info->i2c_dev, tmp, datasizelength);
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s : No NFC Chip!\n", __func__);
...
static int __init s3fhrn2_init(void)
{
	...
	pr_info("Loading s3fhrn2_init driver\n");
